...
export default function EditInvoicePage() {
  ...
  const { data: vehiclesData } = useQuery({
    ...
    enabled: !!selectedCustomer || !!invoice,
  });
  ...
  useEffect(() => {
    if (!dueDateManual && invoiceDate && paymentTerms && paymentTerms !== 'custom') {
  ...
  useEffect(() => {
    if (invoice && !isLoading) {
      ...
      if (invoice.terms || invoice.payment_terms) {
        const t = PAYMENT_TERMS.find(t => t.label === invoice.terms || t.value === invoice.payment_terms);
      ...
      const dPercent = invoice.discount_percentage ? parseFloat(invoice.discount_percentage) : 0;
  ...
  const onSubmit = (data: InvoiceUpdateFormData, status?: string, redirectMode: 'list' | 'payment' = 'list') => {
    setServerError(null);
