...
export default function EditVendorExpensePage() {
  ...
  const { register, control, handleSubmit, watch, setValue, setError, reset } =
    useForm<VendorExpenseForm>();
  ...
  useEffect(() => {
    ...
    reset({
      ...
      notes: expense.notes || "",
      line_items: (expense.line_items ?? []).map((line) => ({
  ...
  const updateMutation = useMutation({
    mutationFn: (payload: Record<string, unknown>) => billingApi.vendorExpenses.update(id, payload),
    onSuccess: () => {
      toast({ title: "Expense updated", description: "Vendor expense saved successfully." });
      router.push(`/billing/expenses/${id}`);
    },
