  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <PrintLayout
        ...
        metaInfo={
          <>
            <div className="mb-1"><span className="font-bold text-foreground">Date:</span> {format(new Date(workOrder.created_at), 'MMM dd, yyyy')}</div>
            <div className="mb-1"><span className="font-bold text-foreground">Status:</span> <span className="uppercase font-semibold">{workOrder.status.replace('_', ' ')}</span></div>
            {workOrder.completed_at && (
              <div><span className="font-bold text-foreground">Completed:</span> {format(new Date(workOrder.completed_at), 'MMM dd, yyyy')}</div>
            )}
          </>
        }
      >
      ...
      `}</style>
        ...
        <div className="grid grid-cols-2 gap-6 mb-8 text-sm">
          <div>
            <div className="font-bold text-foreground uppercase mb-1">Customer</div>
            <div className="font-semibold text-lg">{customerName}</div>
          </div>
          <div>
            <div className="font-bold text-foreground uppercase mb-1">Vehicle</div>
          ...
          <div className="mb-8">
            ...
            <table className="w-full text-sm border-collapse">
              ...
              <tbody>
                {tasks.map((task) => (
                  <tr key={task.id}>
                    <td className="border border-border px-3 py-2">{task.description}</td>
                    <td className="border border-border px-3 py-2 capitalize">{task.status.replace('_', ' ')}</td>
              ...
              <tfoot>
                <tr className="bg-muted font-semibold">
                  <td colSpan={2} className="border border-border px-3 py-2 text-right">Total Hours:</td>
                  <td className="border border-border px-3 py-2 text-right">
            ...
            <table className="w-full text-sm border-collapse">
              ...
              <tbody>
                {parts.map((part) => {
                  ...
                  return (
                    <tr key={part.id}>
                      <td className="border border-border px-3 py-2">{part.part_number || 'N/A'}</td>
                      <td className="border border-border px-3 py-2">{part.part_name || part.description || 'N/A'}</td>
                      <td className="border border-border px-3 py-2 text-right">{quantity}</td>
                      <td className="border border-border px-3 py-2 text-right">{formatCurrency(unitCost)}</td>
                      <td className="border border-border px-3 py-2 text-right">{formatCurrency(lineTotal)}</td>
                    </tr>
                  );
                })}
              </tbody>
              <tfoot>
                <tr className="bg-muted font-semibold">
                  <td colSpan={4} className="border border-border px-3 py-2 text-right">Total Parts:</td>
                  <td className="border border-border px-3 py-2 text-right">{formatCurrency(totalPartsCost)}</td>
                </tr>
              </tfoot>
            </table>
          </div>
        ...
        {/* Footer Notes */}
        <div className="grid grid-cols-2 gap-8 mt-12 pt-8 border-t border-border page-break-avoid">
          <div>
