...
export default function EditVehiclePage() {
  ...
  useEffect(() => {
    if (vehicle && !isLoading) {
      reset({
        ...
        vehicle_type: (vehicle as any).vehicle_type || "saloon",
      });
      if (vehicle.image) {
  ...
  const onSubmit = async (data: VehicleFormData) => {
    setServerError(null);
    try {
      // Check if vehicle with this VIN already exists (and it's not the current vehicle)
      if (data.vin && data.vin.length === 17 && vehicle?.vin !== data.vin.toUpperCase()) {
        try {
          const vinCheck = await vehiclesApi.decodeVin(data.vin.toUpperCase());
          if (vinCheck.success && vinCheck.exists && vinCheck.vehicle) {
            // Only warn if it's a different vehicle
            if (vinCheck.vehicle_id && vinCheck.vehicle_id !== vehicleId) {
              ...
              if (typeof window !== 'undefined' && confirm(`A vehicle with this VIN already exists.\n\n${vehicleDisplayName}\n\nWould you like to view the existing vehicle instead?`)) {
      ...
      // Check if vehicle with this license plate already exists (and it's not the current vehicle)
      if (data.license_plate && data.license_plate.trim() && vehicle?.license_plate?.toUpperCase() !== data.license_plate.trim().toUpperCase()) {
        try {
          const licensePlateCheck = await vehiclesApi.checkLicensePlate(data.license_plate.trim().toUpperCase(), vehicleId);
          if (licensePlateCheck.success && licensePlateCheck.exists && licensePlateCheck.vehicle) {
            // Only warn if it's a different vehicle
            if (licensePlateCheck.vehicle_id && licensePlateCheck.vehicle_id !== vehicleId) {
              ...
              if (typeof window !== 'undefined' && confirm(`A vehicle with license plate "${data.license_plate.toUpperCase()}" already exists.\n\n${vehicleDisplayName}\n\nWould you like to view the existing vehicle instead?`)) {
      ...
      if (imageFile) {
        ...
        payload = formData;
      } else if (!imagePreview && vehicle?.image) {
        const formData = new FormData();
        Object.keys(data).forEach((key) => {
          const value = data[key as keyof VehicleFormData];
          if (value !== undefined && value !== null) {
